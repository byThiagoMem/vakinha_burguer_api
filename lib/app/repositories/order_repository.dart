...
class OrderRepository {
  Future<int> save(OrderViewModel order) async {
    ...
    try {
      ...
      await conn.transaction((_) async {
        final orderResult = await conn!.query(''' insert into pedido(usuario_id, cpf_cliente, endereco_entrega, status_pedido)
        values(?,?,?,?)
        ''', [
          order.userId,
          order.cpf,
  ...
  Future<void> upddateTransactionId(int orderId, String transactionId) async {
    MySqlConnection? conn;
    try {
      conn = await Database().openConnection();
      await Future.delayed(Duration(seconds: 1));
